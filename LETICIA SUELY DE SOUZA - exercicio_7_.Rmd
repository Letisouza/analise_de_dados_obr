...
```{r}
reg1_int <- lm(Q1607 ~ Q18 * Q1501 + D1A_ID + D9 + D2_SEXO + D12A, banco)
summary(reg1_int)
...
```{r}
reg2_int <- lm(Q1607 ~ Q18 * D2_SEXO + Q1501 + D1A_ID + D9 + D12A, banco)
summary(reg2_int)
...
```{r}
reg3_int <- lm(Q1607 ~ Q18 + Q1501 + D1A_ID + D9 +  D2_SEXO * D12A, banco)
summary(reg3_int)
...
Nessa interação, controlando pelas demais variáveis, apenas mulheres amarelas, comparando com homens pretos, possuem significância estatística. Mulheres pardas, também comparando com homens negros, apresentam um coeficiente de efeito médio negativo na aprovação de Bolsonaro.
...
Nesse formato, o gráfico continua trazendo os mesmo resultados, mas agora dividindo por gênero.
